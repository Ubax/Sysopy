...
void do_read(char args[MAX_COMMAND_LENGTH]) {
  ...
  if (f == NULL)
    ERROR_EXIT("opening commands file");
...
void do_echo(char args[MAX_COMMAND_LENGTH]) {
  ...
  receive(&msg);
  if (msg.mType != ECHO)
    MESSAGE_EXIT("Wrong response type");
  printf("%s\n", msg.message);
}
...
void do_init() {
  ...
  msg.senderId = getpid();
  if (mq_send(serverQueueId, (char *)&msg, MESSAGE_SIZE, commandPiority(INIT)))
    ERROR_EXIT("Sending");
  receive(&msg);
  if (msg.mType != INIT)
    MESSAGE_EXIT("Wrong response type");
...
void do_2_operation(enum COMMAND type, char args[MAX_COMMAND_LENGTH]) {
  ...
  int numberOfArguments = sscanf(args, "%s %s", command, text);
  if (numberOfArguments == EOF || numberOfArguments < 2)
    MESSAGE_EXIT("2 command expects text to write");
  send(type, text);
}
...
void cleanExit() {
  if (mq_close(serverQueueId) == -1)
    ERROR_EXIT("Closing server queue");
  if (mq_close(clientQueueId) == -1)
    ERROR_EXIT("Closing client queue");
  if (mq_unlink(queueName) == -1)
    ERROR_EXIT("Deleting queue");
  free(queueName);
}
...
int main(int argc, char **argv) {
  ...
  if ((clientQueueId = mq_open(queueName, O_RDONLY | O_CREAT | O_EXCL, 0666,
                               &queue_attr)) == -1)
    ERROR_EXIT("Opening client queue");
  do_init();
  while (running) {
...
int send(enum COMMAND type, char text[MESSAGE_SIZE]) {
  ...
  if (mq_send(serverQueueId, (char *)&msg, MESSAGE_SIZE,
              commandPiority(type)) == -1)
    ERROR_EXIT("Sending");
  return 0;
}

int receive(struct MESSAGE *msg) {
  if (mq_receive(clientQueueId, (char *)msg, MESSAGE_SIZE, NULL) == -1)
    ERROR_EXIT("Receiving response");
  return 0;
}
