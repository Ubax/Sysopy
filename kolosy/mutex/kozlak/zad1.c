...
static void *threadFunc(void *arg) {
  ...
  s = pthread_mutex_unlock(
      &threadMutex); /*UZUPELNIC: zwolnij mutex threadMutex */
  if (s != 0) {
    printf("%d error 2", s);
    exit(-1);
  }
...
int main(int argc, char *argv[]) {
  ...
  for (idx = 0; idx < argc - 1; idx++) {
    ...
    *index = idx;
    s = pthread_create(&thread[idx].tid, NULL, threadFunc, index);
    if (s != 0) {
  ...
  while (numLive > 0) {
    ...
    while (numUnjoined == 0) {
      /*UZUPELNIC:  czekaj na zmiennej warunkowej threadDied zwiazanej z
             mutekesem threadMutex */
      if (s != 0) {
        printf("%d error 5", s);
        exit(-1);
      }
